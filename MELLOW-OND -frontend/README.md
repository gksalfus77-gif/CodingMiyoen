...
- 전체적인 개별 목록들마다 상품페이지 이며 볼,접시,카페,소품 마다 스타일을 달리주었음.
...
### 📑[상품 상세 페이지]
- 메인 상품 이미지 영역: 좌측에 대형 이미지 슬롯을 배치하여 상품의 디테일과 퀄리티를 한눈에 확인할 수 있도록 했습니다.

- 썸네일 리스트: 메인 이미지 우측에 4개의 세로형 썸네일 슬롯을 구성하여, 사용자가 상품의 다른 각도 컷이나 상세 이미지를 편리하게 전환하며 탐색할 수 있도록 설계했습니다.

- 직관적인 텍스트 배치: 상품명([프리미엄] 올인원 미니 가습기)과 가격(39,000원)을 상단에 굵고 선명하게 노출하여 정보 전달력을 높였습니다. 가격은 붉은색 포인트 컬러를 적용하여 시각적 주목도를 주었습니다.
